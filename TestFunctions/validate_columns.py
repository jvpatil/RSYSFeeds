...
class ValidateColumns(DBFunctions,CommonFunctions,DeviceDetails):
    ...
    def verifyColumns(self):
        ...
        account_id = self.get_account_id(ced_files[0])
        # curs = self.init_db_connection("sysAdmin")
        curs = self.start_db_connection(paths.pod,"sysAdmin")
        ced_columns_from_db,built_in_headers_from_db = self.get_headers_from_db(curs, account_id)
        result_file=self.write_headers_to_file(account_id, ced_columns_from_db, "ced_columns_from_db")
        self.close_db_connection( curs)

        ced_columns_from_podconfig = self.get_headers_from_podconfig()
        result_file= self.write_headers_to_file(account_id, ced_columns_from_podconfig, "ced_columns_from_podconfig")

        # syslocalCust_curs = Setup.init_db_connection( "syslocalCust")
        ...
        for file in ced_files:
            ...
            if  not self.is_file_empty(file):
                delimiter,qoute_char = self.get_file_info(file)
                # ced_columns_from_file = self.get_headers_from_ced(file)
                ced_columns_from_file = self.get_headers_from_ced(file,delimiter,qoute_char)
                self.write_headers_to_file(account_id, ced_columns_from_file,"ced_columns_from_file")
                report = self.validate_columns_and_save_result(account_name,account_id, file, ced_columns_from_file,
        ...
        self.print_results_of_column_validation( status_report, total_time, len(ced_files), empty_files)
